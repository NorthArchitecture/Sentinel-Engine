...
// Unauthorized use prohibited. See LICENSE for terms.
// Contact: contact.northprotocol@proton.me
...
pub mod vk {
    ...
    static WITHDRAW_IC: [G1; 5] = [
        ...
        [
            0x23, 0x38, 0xa4, 0x36, 0xfd, 0x31, 0xaf, 0x70, 0x86, 0xec, 0xa6, 0xeb, 0x9b, 0x67, 0xc8, 0x9c,
            0x63, 0x90, 0xf7, 0x8b, 0xec, 0x2c, 0x2e, 0x72, 0xb2, 0xea, 0x11, 0x3c, 0x4c, 0x1e, 0x0f, 0x44,
...
pub mod sentinel {
    ...
    /// Prepares the rail's SOL vault to receive confidential transfers (vault→vault).
    /// Call this instead of depositing first; do not deposit before receiving or commitment will mismatch.
    pub fn prepare_receive_sol(
        ctx: Context<PrepareReceiveSol>,
        ...
        sol_asset_state.asset_key = sol_asset_key();
        sol_asset_state.balance_commitment = initial_commitment;
        sol_asset_state.encrypted_balance = [0u8; 64];
    ...
    pub fn confidential_transfer(
        ...
        transfer_record.amount_lamports = amount;
        transfer_record.created_at = clock.unix_timestamp;
        transfer_record.bump = ctx.bumps.transfer_record;

        // Ensure receiver's vault pool exists (e.g. first transfer to this rail)
        ensure_vault_pool_initialized(
            &ctx.accounts.authority,
            &ctx.accounts.receiver_vault_pool.to_account_info(),
            &ctx.accounts.system_program,
            &ctx.accounts.receiver_rail.key(),
            ctx.bumps.receiver_vault_pool,
        )?;

        // Move SOL from sender's vault pool to receiver's — so receiver can withdraw
        let sender_pool = ctx.accounts.sender_vault_pool.to_account_info();
        let receiver_pool = ctx.accounts.receiver_vault_pool.to_account_info();
        require!(sender_pool.lamports() >= amount, SentinelError::InsufficientVaultBalance);
        **sender_pool.try_borrow_mut_lamports()? -= amount;
        **receiver_pool.try_borrow_mut_lamports()? += amount;
    ...
    pub fn confidential_transfer_token(
        ...
        transfer_record.amount_lamports = 0;
        transfer_record.created_at = clock.unix_timestamp;
        transfer_record.bump = ctx.bumps.transfer_record;
...
pub struct TransferRecord {
    ...
    pub is_token: bool,
    pub token_mint: Pubkey,
    /// SOL amount (lamports) for receiver — so balance updates automatically like a bank account
...
#[derive(Accounts)]
pub struct PrepareReceiveSol<'info> {
    #[account(has_one = authority @ SentinelError::Unauthorized)]
    pub rail: Account<'info, RailState>,
    #[account(
        seeds = [b"zk_vault", rail.key().as_ref()],
...
pub struct ConfidentialTransfer<'info> {
    ...
    #[account(
        ...
        bump,
        constraint = *sender_vault_pool.owner == crate::ID @ SentinelError::InvalidVaultPoolOwner,
    )]
    /// CHECK: PDA vault pool
    pub sender_vault_pool: UncheckedAccount<'info>,
    #[account(
        mut,
        seeds = [b"vault_pool", receiver_rail.key().as_ref()],
        bump,
        constraint = *receiver_vault_pool.owner == crate::ID @ SentinelError::InvalidVaultPoolOwner,
    )]
    /// CHECK: PDA vault pool
